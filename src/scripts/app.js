...
const DateUtil = {
  ...
  long(d) { return d.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); },
  short(d) { return d.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }); },
...
function ratingBadge(destId, tone = 'dark') {
  ...
  const count = Number(r.count).toLocaleString('id-ID');
  const label = `Rating resmi ${score} dari 5, berdasarkan ${count} ulasan${r.source ? ` di ${r.source}` : ''}`;
...
const Auth = {
  ...
  resumeIntent() {
    ...
    setTimeout(() => {
      ...
      Toast.show('Selamat datang kembali — lanjutkan pemesanan Anda');
    }, 150);
  },
      ...
      <div class="moment-slide absolute inset-0 transition-opacity duration-700 ${i === 0 ? 'opacity-100' : 'opacity-0'}">
        ...
        <div class="absolute inset-0 bg-bark/45 flex items-end p-8">
          <div>
            ...
            <p class="text-white font-heading font-semibold text-lg">${esc(m.title)}</p>
          </div>
        </div>
      </div>`).join('')
...
const WahanaFilter = {
  init() {
    ...
    const apply = cat => {
      ...
      countEl.textContent = `Menampilkan ${visible} dari ${cards.length} wahana`;
...
const Destinations = {
  ...
  apply() {
    ...
    document.getElementById('destCount').textContent = `Menampilkan ${visible} dari ${cards.length} destinasi`;
        ...
        ${rating ? `
          <div class="flex items-center gap-3 mb-5 pb-5 border-b border-line-soft">
            ...
            <span class="text-xs text-muted">
              Rating resmi${rating.source ? ` · ${esc(rating.source)}` : ''}
            </span>
          </div>` : ''}

        <p class="text-sm text-muted leading-relaxed">${esc(d.desc)}</p>

        <p class="font-heading font-semibold text-ink text-sm mt-6 mb-3">Sorotan</p>
        <div class="flex flex-wrap gap-2">
          ${d.highlights.map(h =>
            `<span class="text-xs bg-paper border border-line-soft text-muted px-3 py-1.5 rounded-full">${esc(h)}</span>`).join('')}
        </div>

        <p class="font-heading font-semibold text-ink text-sm mt-6 mb-3">Galeri</p>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mt-7">
          ${d.bookable
            ? `<button type="button" data-dest-book class="btn-accent flex-1 py-3.5">
                 ${icon('ticket', 'w-4 h-4')} Beli Tiket ${esc(d.name)}
               </button>`
            : `<a href="https://wa.me/6281111121162" target="_blank" rel="noopener" class="btn-accent flex-1 py-3.5">
                 Tanya Ketersediaan
               </a>`}
          <button type="button" data-close-dest class="btn-outline py-3.5">Tutup</button>
        </div>
      </div>`;
        ...
        <div class="p-5 flex flex-col flex-1">
          ...
          <div class="flex flex-wrap gap-1.5 mt-4">
            <span class="inline-flex items-center gap-1 text-[11px] bg-paper border border-line-soft text-muted px-2.5 py-1 rounded-full">
              ${icon('users', 'w-3 h-3')} ${r.cap} orang
            </span>
  ...
  init() {
    ...
    grid.innerHTML = GALLERY.map((g, i) => `
      <button type="button" data-index="${i}"
        ...
        <span class="absolute left-3 bottom-3 text-white text-xs font-heading font-medium opacity-0 group-hover:opacity-100 transition-opacity">${esc(g.title)}</span>
      </button>`).join('');
    ImageFallback.bindAll(grid);
...
const Booking = {
  ...
  init() {
    ...
    document.getElementById('dateHint').textContent =
      'Tanggal tercepat yang bisa dipilih: ' + DateUtil.long(minDate) + '.';
    ...
    document.getElementById('stayHint').textContent =
      'Check-in tercepat: ' + DateUtil.long(minDate) + '. Minimal menginap 1 malam.';
  ...
  updateTicket() {
    ...
    document.getElementById('adultCount').textContent = t.adult;
    document.getElementById('childCount').textContent = t.child;
    document.querySelectorAll('[data-step]').forEach(btn => {
      const key = btn.dataset.step;
      const delta = Number(btn.dataset.delta);
      const min = key === 'adult' ? 1 : 0;
      btn.disabled = (delta < 0 && t[key] <= min) || (delta > 0 && t[key] >= RULES.MAX_TICKETS);
    });
    ...
    let arrival = null;
    if (t.date) {
      arrival = DateUtil.parseISO(t.date);
      const min = DateUtil.addDays(DateUtil.today(), RULES.MIN_LEAD_DAYS);
      if (!arrival) {
        dateError.textContent = 'Format tanggal tidak terbaca. Pilih ulang tanggal kedatangan.';
        dateError.classList.remove('hidden');
      } else if (arrival < min) {
        dateError.textContent = 'Tanggal kedatangan minimal H-1 dari hari ini. Pemesanan untuk hari yang sama atau tanggal lampau tidak tersedia.';
        dateError.classList.remove('hidden');
        arrivalInput.value = '';
        t.date = '';
        arrival = null;
    ...
    document.getElementById('adultPriceLabel').textContent = rupiah(quote.adultUnit) + ' / orang';
    document.getElementById('childPriceLabel').textContent = rupiah(quote.childUnit) + ' / orang';
    ...
    if (valid) {
      document.getElementById('expiryValue').textContent =
        DateUtil.long(quote.expiry) + ` (kedatangan + ${RULES.TICKET_VALID_DAYS} hari)`;
    }

    document.getElementById('sumPackage').textContent =
      this.getPackage().name + (t.nationality === 'manca' ? ' · Mancanegara' : ' · Domestik');
    document.getElementById('sumArrival').textContent = valid
      ? DateUtil.long(arrival) + ' · ' + (DateUtil.isWeekend(arrival) ? 'Weekend' : 'Weekday')
      : 'Belum dipilih';
    document.getElementById('sumExpiry').textContent = valid ? DateUtil.long(quote.expiry) : 'Belum dipilih';
    document.getElementById('sumAdult').textContent =
      `${t.adult} × ${rupiah(quote.adultUnit)} = ${rupiah(quote.adultUnit * t.adult)}`;
    document.getElementById('sumChild').textContent = t.child === 0
      ? '—'
      : `${t.child} × ${rupiah(quote.childUnit)} = ${rupiah(quote.childUnit * t.child)}`;
  ...
  async checkoutTicket() {
    ...
    lockScroll(true);
    document.getElementById('eticketLoadingText').textContent = 'Memproses pembayaran…';
    ...
    if (!ok) {
      ...
      errEl.textContent = data?.error ?? 'Pemesanan gagal. Coba lagi.';
  ...
  renderEticket(b) {
    ...
    document.getElementById('ticketQty').textContent =
      `${b.quantity} orang (${t.adult} dewasa${t.child ? ' + ' + t.child + ' anak' : ''})`;
  ...
  async downloadTicket() {
    ...
    try {
      ...
      Toast.show('Gagal membuat file. Coba lagi.');
    } finally {
      btn.disabled = false;
  ...
  shareWhatsApp() {
    ...
    const lines = [
      ...
      `*Nama:* ${get('ticketName')}`,
      `*Tipe Tiket:* ${get('ticketType')}`,
      `*Jumlah:* ${get('ticketQty')}`,
      `*Total:* ${get('ticketTotal')}`,
      `*Check-in:* ${get('ticketArrival')}`,
      `*Berlaku s/d:* ${get('ticketExpiry')}`,
      '',
      `Tiket ini berlaku maksimal ${RULES.TICKET_VALID_DAYS} hari sejak tanggal kedatangan.`,
    ];
  ...
  updateRoom() {
    ...
    document.getElementById('roomRateLabel').textContent = rupiah(room.rate) + ' per kamar / malam';
    document.getElementById('guestCapLabel').textContent = `Kapasitas total ${room.cap * s.rooms} orang`;
    ...
    if (s.checkIn && s.checkOut) {
      ...
      if (!inDate || !outDate) {
        stayError.textContent = 'Format tanggal tidak terbaca. Pilih ulang tanggal menginap.';
        stayError.classList.remove('hidden');
      } else if (inDate < min) {
        stayError.textContent = 'Check-in paling cepat besok (H-1). Tanggal hari ini atau lampau tidak tersedia.';
        stayError.classList.remove('hidden');
        document.getElementById('checkInDate').value = '';
        s.checkIn = '';
      } else if (outDate <= inDate) {
        stayError.textContent = 'Tanggal check-out harus setelah check-in — minimal menginap 1 malam.';
        stayError.classList.remove('hidden');
      } else {
    ...
    if (overCap) {
      capWarn.textContent =
        `${s.guests} tamu melebihi kapasitas ${room.cap * s.rooms} orang. Tambah jumlah kamar atau kurangi tamu.`;
    }
    ...
    document.getElementById('rsumHotel').textContent = room.hotelName;
    document.getElementById('rsumRoom').textContent = `${room.name} · ${room.type}`;
    document.getElementById('rsumIn').textContent = valid ? DateUtil.long(inDate) : 'Belum dipilih';
    document.getElementById('rsumOut').textContent = valid ? DateUtil.long(outDate) : 'Belum dipilih';
    document.getElementById('rsumNights').textContent = valid ? `${nights} malam` : '—';
    document.getElementById('rsumGuests').textContent = `${s.guests} tamu · ${s.rooms} kamar`;
    document.getElementById('rsumCalc').textContent =
      valid ? `${rupiah(room.rate)} × ${nights} malam × ${s.rooms} kamar` : '—';
  ...
  async checkoutRoom() {
    ...
    document.getElementById('successTitle').textContent = 'Pemesanan Kamar Berhasil';
    document.getElementById('successDetail').textContent =
      `${b.rooms} × ${b.room_name} di ${b.hotel_name} untuk ${b.guests} tamu, ` +
      `${DateUtil.short(DateUtil.parseISO(b.check_in))} – ${DateUtil.short(DateUtil.parseISO(b.check_out))} ` +
      `(${b.nights} malam). Total ${rupiah(Number(b.total_price))}.`;
    document.getElementById('bookingCode').textContent = b.booking_code;

    this.close();
    document.getElementById('successModal').classList.remove('hidden');
    lockScroll(true);
    Toast.show('Pemesanan kamar berhasil dibuat');
  },
};
...
const ESS = {
  ...
  async doLogin() {
    ...
    if (!nik || !password) {
      err.textContent = 'NIK dan password wajib diisi.';
  ...
  async loadTickets() {
    ...
    if (!ok) {
      ...
      document.getElementById('essSyncNote').textContent =
        'Gagal memuat data: ' + (data?.error ?? 'kesalahan jaringan');
      return;
    }
  ...
  renderTickets() {
    ...
    document.getElementById('essTicketBody').innerHTML = list.map(t => {
      ...
      const action = t.status === 'LUNAS'
        ? `<button type="button" data-verify="${esc(t.booking_code)}"
             ...
             ${icon('badge-check', 'w-3.5 h-3.5')} Verifikasi</button>`
    ...
    document.getElementById('essSyncNote').textContent =
      'Diperbarui ' + new Date().toLocaleTimeString('id-ID') + ' · menyegarkan tiap 20 detik';
  },
  ...
  statusBadge(s) {
    ...
    return `<span class="inline-flex items-center gap-1 text-[11px] font-heading font-semibold border px-2.5 py-1 rounded-full whitespace-nowrap ${cls}">${
      s === 'TERPAKAI' ? 'TERPAKAI (USED)' : esc(s)}</span>`;
  },
  ...
  async confirmVerify() {
    ...
    const { ok, status, data } = await api('/api/ess/verify', {
      method: 'POST',
      body: JSON.stringify({ booking_code: t.booking_code }),
    });
    ...
    if (!ok) {
      if (status === 401) { this.forceLogout(); return; }
      Toast.show(data?.error ?? 'Gagal memperbarui status tiket');
      this.closeVerify();
  ...
  forceLogout() {
    clearInterval(this.pollTimer);
    Toast.show('Sesi karyawan berakhir. Silakan masuk kembali.');
